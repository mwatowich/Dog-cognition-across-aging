...
# Model matrix of the truncation hypothesis (additive)
...
# Model matrix of the compression hypothesis (interactive)
...
# Model of truncation hypothesis 
...
# Model of compression hypothesis 
compression_out <- pqlseq(RawCountDataSet = t(success.counts), 
                         Phenotypes = compression_cov[,2],    
                         Covariates =  compression_cov[,-2], 
                         RelatednessMatrix = GRM, 
                         LibSize = t(total.counts), 
                         fit.model = 'BMM', 
                         fit.method = 'REML') 

# calculate AIC from loglik output, compare AIC of the truncation and compression models 
...
truncation_delay_grat <- cbind(truncation_delay_grat$betahat,             # betas
                               truncation_delay_grat$varbetahat,          # se
                               truncation_delay_grat$pvalbeta[, "none"],  # p-values 
                               truncation_delay_grat$loglik)              # log likelihood
colnames(truncation_delay_grat) <- c("betas", "se", "p-values", "loglik")


# Model of compression hypothesis 
compression_delay_grat <- emmreml(y = dog_delay_grat$PC1,
                                 X = compression_cov,
                                 Z = Zmatrix,
                                 K = IBD,
                                 varbetahat = T, varuhat = T, PEVuhat = T, test = T)

compression_delay_grat <- cbind(compression_delay_grat$betahat, 
                               compression_delay_grat$varbetahat, 
                               compression_delay_grat$pvalbeta[, "none"], 
                               compression_delay_grat$loglik)
colnames(compression_delay_grat) <- c("betas", "se", "p-values", "loglik")
